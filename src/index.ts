...
export async function getAnime(id: number): Promise<AnimeWithRelations | null> {
  try {
    ...
    const studioData = await db
      .select()
      .from(animeStudios)
      .where(eq(animeStudios.animeId, id));
    ...
    const relationData = await db
      .select()
      .from(relations)
      .where(eq(relations.animeId, id));

    const synonymData = await db
      .select()
      .from(animeSynonyms)
      .where(eq(animeSynonyms.animeId, id));
    ...
    const searchResultData = await db
      ...
      .innerJoin(
        matchResults,
        and(
          eq(matchResults.animeId, id),
          eq(matchResults.searchResultId, searchResults.id)
        )
      )
