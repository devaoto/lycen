...
async function fetchIds(): Promise<number[]> {
  try {
    const res = await lycen.get<string>("https://raw.githubusercontent.com/5H4D0WILA/IDFetch/main/ids.txt");
    const ids = res.data.split("\n").map(Number).filter(id => !Number.isNaN(id));
...
async function updateExistingMappings() {
  try {
    const activeAnime = await db.select()
      .from(animes)
      .where(
        not(
            // @ts-expect-error
          or(
            eq(animes.status, 'FINISHED'),
            eq(animes.status, 'CANCELLED')
          )
        )
      );
    ...
    for (const anime of activeAnime) {
      try {
        ...
        logger.error(chalk.red(`Error updating mapping for ID ${anime.id}: ${(error as Error).message}`));
      }
    }
  } catch (error) {
